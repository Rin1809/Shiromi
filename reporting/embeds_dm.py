...
import datetime
import logging
from typing import Dict, Any, Optional, Union, Tuple, List, Set, Callable # Thêm Callable
...
PERSONAL_CHANNEL_CHART_LIMIT = 3 # Giới hạn số kênh hiển thị trong chart DM
...
async def create_personal_activity_embed(
    ...
    if user_channel_msg_counts:
        ...
        if sorted_channels:
            ...
            # Thêm chart vào đầu
            if bar_chart_str:
                scope_lines.insert(1, bar_chart_str) # Chèn vào vị trí thứ 2
                scope_lines.insert(2, "") # Thêm dòng trống sau chart

            # Thêm danh sách text như cũ (có thể tùy chọn bỏ đi nếu chart đủ)
            # scope_lines.append("📍 **Top Kênh/Luồng Hoạt Động:**")
            # num_top_channels = len(sorted_channels[:PERSONAL_CHANNEL_CHART_LIMIT]) # Giới hạn text list theo chart
            # for i, (location_id, msg_count_in_loc) in enumerate(sorted_channels[:PERSONAL_CHANNEL_CHART_LIMIT]):
            #     channel_obj = guild.get_channel_or_thread(location_id)
    ...
    if user_custom_emoji_counts:
        ...
        for emoji_id, count in sorted_emojis:
            ...
            if emoji_obj: emoji_strs.append(f"{str(emoji_obj)} ({count:,})")
            else: emoji_strs.append(f"`ID:{emoji_id}` ({count:,})")
        if emoji_strs: top_items_lines.append(f"{e('mention')} **Top Emoji Server:** " + " ".join(emoji_strs))
    ...
    if user_sticker_counts:
        ...
        for sticker_id_str, count in sorted_stickers:
            if sticker_id_str.isdigit():
                sticker_id = int(sticker_id_str); sticker_name = sticker_name_cache.get(sticker_id, "...")
                sticker_strs.append(f"'{utils.escape_markdown(sticker_name)}' ({count:,})")
            else: sticker_strs.append(f"`ID:{sticker_id_str}` ({count:,})")
        if sticker_strs: top_items_lines.append(f"{e('sticker')} **Top Stickers:** " + ", ".join(sticker_strs))
    ...
    if user_hourly_counter:
        ...
        if hourly_grouped:
             timezone_str = "UTC"; local_offset_hours = 0
             if hasattr(utils, 'get_local_timezone_offset'):
                 local_offset_hours = utils.get_local_timezone_offset(); timezone_str = f"UTC{local_offset_hours:+d}"
             elif hasattr(utils, 'local_timezone_offset_hours'):
                 local_offset_hours = utils.local_timezone_offset_hours; timezone_str = f"UTC{local_offset_hours:+d}"
