...
async def _try_add_generic_leaderboard(embed_list, func, counter_data, **kwargs):
    ...
    try:
        embed = await func(counter_data, **kwargs)
...
async def generate_and_send_reports(scan_data: Dict[str, Any]):
    ...
    kwargs_basic = {"guild": server, "bot": bot, "scan_errors_ref": scan_errors}
    ...
    # Thêm BXH người thả reaction nhiều nhất
    try:
        embed = await embeds_analysis.create_top_reaction_givers_embed(
    ...
    except Exception as ex: err = f"Lỗi embed create_top_reaction_givers_embed: {ex}"; log.error(f"{e('error')} {err}", exc_info=True); scan_errors.append(err)
    ...
    # 3. Bảng Xếp Hạng Sáng Tạo Nội Dung & Emoji/Sticker
    log.info(f"--- {e('image')} BXH Sáng tạo Nội dung & Items ---")
    ...
    try: # Top Custom Emoji Users (Content)
    ...
    if content_creation_embeds:
        await _send_report_embeds(scan_data, content_creation_embeds, "BXH Sáng tạo Nội dung & Items")
    ...
    try: # oldest members
        ...
        log.error(f"{e('error')} {err}", exc_info=True); scan_errors.append(err)
    ...
    except Exception as ex: error_msg = f"Lỗi embed create_error_embed: {ex}"; log.error(f"{e('error')} {error_msg}", exc_info=True); scan_errors.append(error_msg)
